...
function MyNavBar() {
 let navigate=useNavigate()
  ...
  let navItems=['Home', 'Product','Listings', 'Create Listing', 'About', 'Contact']
  return (
<>
<div
     ...
     <div className="d-none d-lg-flex  mx-2 ">
      ...
      {navItems.map((val)=>  <strong className='navitems mx-4' onClick={()=> val==='Create Listing'? navigate('/createlisting'):navigate(`/${val.toLocaleLowerCase()}`)}>{val}</strong>
      )}
     ...
     */}
      ...
      onClick={()=>navigate('/')}
      className='navBtn d-sm-block d-none'>My Account</strong>
    </div >
